...
/**
 * Universal proxy handler that forwards requests to the container-internal Scheduler Service (:3020).
 */
async function forwardToSchedulerService(req: Request, res: Response): Promise<void> {
  const schedulerBaseUrl = process.env.SCHEDULER_SERVICE_URL || 'http://127.0.0.1:3020';
  const targetUrl = `${schedulerBaseUrl}${req.originalUrl}`;
  ...
  try {
    ...
    log.error('Failed to proxy request to internal scheduler service', {
      method: req.method,
      targetUrl,
      error: err.message,
    });
    res.status(502).json({
      error: 'Scheduler service gateway error',
      message: err.message,
    });
  }
}

// Register all metadata and scheduler endpoints on Gateway
...
router.use('/schedule', forwardToSchedulerService);
